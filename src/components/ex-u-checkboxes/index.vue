...
import UCheckbox from "@lcap-ui/src/components/u-checkbox.vue";
...
export default {
  ...
  mixins: [MParent, MField, MConverter, SupportDataSource, MPreview, sync({
    data() {
  ...
  computed: {
    ...
    isCheckAll() {
      // 当隐藏子项时，基于数据源计算全选状态
      if (this.hideItems && this.currentDataSource && this.currentDataSource.data) {
        const allValues = this.currentDataSource.data.map(item => this.$at2(item, this.valueField));
        return allValues.every(value => this.currentValue.includes(value));
      }
  ...
  methods: {
    ...
    toggleCheckAll(all) {
      ...
      // 当隐藏子项时，基于数据源进行全选/取消全选操作
      if (this.hideItems && this.currentDataSource && this.currentDataSource.data) {
        const allValues = this.currentDataSource.data.map(item => this.$at2(item, this.valueField));
        if (all) {
          // 全选：将所有值添加到currentValue中
    ...
    setCheckAll() {
      ...
      if (this.hideItems && this.currentDataSource && this.currentDataSource.data) {
        ...
        const selectedCount = allValues.filter(value => this.currentValue.includes(value)).length;
