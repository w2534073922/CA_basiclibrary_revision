...
namespace extensions.vue_library_radio_coreagent.viewComponents {
  ...
  @ExtensionComponent({
      ...
      ideusage: {
        ...
        dataSource: {
          ...
          loopElem: "> label[class*='radio']:not([data-nodepath])",
          emptySlot: {
            display: 'inline',
        ...
        slotWrapperInlineStyle: {
          item: 'display:inline-flex;align-items:center;min-width:90px;white-space:nowrap;overflow:hidden;',
        },
      },
    })
  @Component({
  ...
  export class ExURadiosOptions<T, V> extends ViewComponentOptions {
    ...
    dataSchema: T;
    @Prop<ExURadiosOptions<T, V>, 'textField'>({
      group: '数据属性',
      title: '选项文本字段',
    ...
    onBeforeSelect: (event: { value: V; oldValue: V }) => any;
    @Event({
      title: '输入时',
    ...
    onSelect: (event: { value: V; oldValue: V }) => any;
    @Event({
      title: '改变后',
